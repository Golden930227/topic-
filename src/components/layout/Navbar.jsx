...
function Navbar({ user, onAuthButtonClick }) {
  ...
  const dropdown = (name, label, children) => (
    <div className="small-dropdown">
      <button type="button" className={`small-dropdown-button ${openMenu === name ? "active" : ""}`} onClick={() => setOpenMenu(openMenu === name ? null : name)} aria-expanded={openMenu === name}>
        {label}<span className="dropdown-arrow">⌄</span>
      </button>
      {openMenu === name && <div className="small-dropdown-menu">{children}</div>}
    </div>
  )
  ...
  return (
    <header className="navbar" ref={navbarRef}>
      ...
      >
        ...
        <a href="https://ee.nfu.edu.tw/zh_tw/teacher01/faculty/%E7%8E%8B%E9%B3%B4%E7%AB%8B-Ming-Li-Wang-77332159" target="_blank" rel="noreferrer">專題教授</a>
